...
| 3 | Payment authorization as a record with history | **Built** | `payment_authorizations` (0020): who, mode, wording version, Stripe refs, attributed revocation, one active per account, settled rows immutable by trigger; domain mirror in `paymentAuthorization.ts`. The wording itself is on file: Karynn supplied the full Electronic Payment Authorization text (22 Aug), vendored verbatim as `joy-epay-v1` with a clause-by-clause map to the code that enforces it |
...
| 19 | V1 screens | **Mostly built** | Admin: the Saturday run card walks §7.2 end to end — exceptions, drafts, approval, and SEND: the JH- number is assigned, the §9.4 notification queues, the invoice lands in Outstanding and the reports, and the trail records the send. Client: billing summary/open invoices/preference/history built; method setup and live payment need Stripe Elements (server-layer) |
